...
from Project_3.ppo_fcnn_v2 import (
    Policy,
    Buffer,
    StudentPolicy,
    StudentAgent,
...
os.mkdir(f"./runs/fcnn/{args.layout_name}/{args.exp_name}")
exp_path = rf"./runs/fcnn/{args.layout_name}/{args.exp_name}"
    ...
    for index, env in enumerate(envs_list):
        ...
        for step in range(param["training_param"]["num_steps"]):
            ...
            r_shaped2 = [
                0,
                0,
            ]  # helper_additional_reward_shaping(step, env.base_env.game_stats)
            if env.agent_idx:
            ...
            actor_R = torch.Tensor([r_shaped_0 + R, r_shaped_1 + R]).reshape(-1)
