...
def evaluate_ppo(ppo_policy, env, num_episode=10):
    ...
    for i in range(num_episode):
        ...
        reshaped_obs = helper_func_obs(env, next_obs["overcooked_state"])
        prev_done = torch.zeros((2, 1))
        lstm_prev_h = torch.zeros((2, 256))
        lstm_prev_c = torch.zeros((2, 256))
        first = True
        while not next_done:
            ...
            with torch.no_grad():
                ...
                if first:
                    tem = ppo_policy.forward_lstm_net(
                        ppo_policy.forward_cnn_fcc_net(reshaped_obs),
                        (lstm_prev_h, lstm_prev_c),
                        prev_done,
                    )[0]
...
def default_config():
    ...
    training_params = {
        ...
        # use cuda
        "use_cuda": False,
        "extra_reward": reward_shaping_dict,
    }
...
os.mkdir(f"runs/lstm_v2/{args.layout_name}/{args.exp_name}")
exp_path = rf"runs/lstm_v2/{args.layout_name}/{args.exp_name}"
...
policy.load_model(
    f"runs/lstm_v2/cramped_room/with_extra_reward_4_envs_2_minibathes_2_epoch/5200-model.pt"
)

optimizer_shared = optim.Adam(
    list(policy.cnn_net.parameters())
    + list(policy.fcc_net.parameters())
    + list(policy.lstm_net.parameters()),
...
buffer = Buffer((26, 5, 4), dotdict(param["buffer_param"]))
...
for update in tqdm(range(1, num_updates + 1), total=num_updates):
    ...
    for index, env in enumerate(envs_list):
        ...
        for step in range(param["training_param"]["num_steps"]):
            ...
            r_shaped2 = helper_additional_reward_shaping(step, env.base_env.game_stats)
            if env.agent_idx:
            ...
            next_reshaped_obs = helper_func_obs(env, next_obs["overcooked_state"])

            actor_R = torch.Tensor([r_shaped_0 + R / 2, r_shaped_1 + R / 2]).reshape(-1)
    ...
    for epoch in range(param["training_param"]["epoch"]):
        ...
        for batch_data in data_generator:
            ...
            new_value, _, new_logprobs, new_entropy, _ = policy.get_action_and_value(
                obs_samples,
