...
reward_shaping_dict = {
    ...
    "soup_drop":-1,
    # Potting events
    "optimal_onion_potting": 2,
    "viable_onion_potting": 2,
}


def helper_additional_reward_shaping(current_step, game_stats, reward_dict={}):
    if reward_dict == {}:
        return [0, 0]
    ...
    for event, agents_steps in game_stats.items():
        if event in EVENT_TYPES:
            ...
            if current_step in agents_steps[0]:
                reward_agent1 += reward_dict.get((event), 0)
            if current_step in agents_steps[1]:
                reward_agent2 += reward_dict.get((event), 0)
...
class Policy(nn.Module):
    ...
    def get_action_and_value(self, obs, action=None, deterministic=False):
        ...
        value = self.critic(torch.flatten(hidden))
