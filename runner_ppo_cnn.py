    ...
    for i in range(num_episode):
        ...
        while not next_done:
            ...
            with torch.no_grad():
                ...
                if first:
                    tem = ppo_policy.forward_cnn_fcc_net(reshaped_obs)
...
os.mkdir(f"runs/cnn/{args.layout_name}/{args.exp_name}")
exp_path = rf"runs/cnn/{args.layout_name}/{args.exp_name}"
...
policy.load_model(
    r"./runs/cnn/cramped_room/with_extra_reward_20_envs_1_minibathes_1_epoch/800-model.pt"
)

optimizer_shared = optim.Adam(
    list(policy.cnn_net.parameters()) + list(policy.fcc_net.parameters()),
...
for update in tqdm(range(1, num_updates + 1), total=num_updates):
    ...
    for epoch in range(param["training_param"]["epoch"]):
        ...
        for batch_data in data_generator:
            ...
            new_value, _, new_logprobs, new_entropy = policy.get_action_and_value(
                obs_samples,
