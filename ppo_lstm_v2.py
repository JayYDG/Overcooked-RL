...
def helper_func_obs(env, overcookstate):
    ...
    # if env.agent_idx:
    #     temp_lossless_state[[1, 0], :] = temp_lossless_state[[0, 1], :]
...
class Policy(nn.Module):
        ...
        torch.save(
            {
                ...
                "lstm_net_state_dict": self.lstm_net.state_dict(),
    ...
    def load_model(self, path):
        ...
        self.lstm_net.load_state_dict(params_dict["lstm_net_state_dict"])
...
class Buffer:
    ...
    def __init__(self, obs_space: tuple, args):
        ...
        self.done = torch.zeros_like(
            self.logprobs
        )  # torch.zeros((self.length, self.num_head, 1), dtype=torch.float32)
        self.value = torch.zeros_like(self.logprobs)  # torch.zeros_like(self.done)
        self.lstm_hn = torch.zeros(
            (self.length, self.num_head, self.num_agents, self.lstm_n),
            dtype=torch.float32,
        )
    ...
    def feed_forward_generator(self):
        ...
        b_lstm_hn = self.lstm_hn.reshape((-1,) + self.lstm_hn.shape[3:])
        b_lstm_cn = self.lstm_cn.reshape((-1,) + self.lstm_cn.shape[3:])
        b_done = self.done.reshape((-1,) + self.value.shape[3:])
...
class StudentPolicy(NNPolicy):
    ...
    def __init__(self, policy, base_env, action_space):
        ...
        self.action_space = action_space
        self.lstm_prev_h = torch.zeros(2, 256)
        self.lstm_prev_c = torch.zeros(2, 256)
        self.done = torch.zeros((2, 1))
    ...
    def state_policy(self, state, agent_index):
        ...
        with torch.no_grad():
            _, actions, _, _, lstm_next = self.policy.get_action_and_value(
                reshaped_obs,
        ...
        self.lstm_prev_h = lstm_next[0]
        self.lstm_prev_c = lstm_next[1]
